...
export function TemplatesClient({ initialTemplates }: TemplatesClientProps) {
  // Fallback to static dummy data if the database table is empty for demonstration purposes.
  // In a full production launch, we would seed the DB and only use initialTemplates.
  const templates = initialTemplates.length > 0 ? initialTemplates : [
        ...
        {templates.map((t, i) => (
          <div key={i} className="group flex flex-col glass-card rounded-3xl border border-border overflow-hidden hover:border-foreground/30 transition-all hover:shadow-lg bg-background">
            <div className="aspect-16/10 overflow-hidden relative border-b border-border">
              {/* Using native img for template thumbnails from remote dynamic sources */}
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src={t.thumbnail || t.thumbnailUrl} className="w-full h-full object-cover grayscale opacity-40 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-700" alt={t.title} />
            ...
            <div className="p-8 flex flex-col grow bg-accent/5">
